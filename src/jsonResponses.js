...
const getSongs = (request, response) => {
  ...
  lowRoarData.forEach((album) => {
    album.Tracks.forEach((track) => {
      ...
      if (track.Rating !== undefined) trackData.Rating = track.Rating;
      if (!track.TrackArtist) trackData.Artist = album.AlbumArtist;
...
// Uses title and year inputs from the user to search for the appropriate album.
...
const songSearch = (request, response) => {
  ...
  lowRoarData.forEach((album) => {
    if (yearSearchTerm && !album.Released.toString().includes(yearSearchTerm)) {
      return;
    }

    album.Tracks.forEach((track) => {
      if (!track.Name) return;
      if (titleSearchTerm && !track.Name.toLowerCase().includes(titleSearchTerm)) return;
...
const addRating = (request, response) => {
  ...
  lowRoarData.forEach((album) => {
    if (foundSong) return; // skip the rest once found

    album.Tracks.forEach((track) => {
      if (foundSong) return; // skip inner loop too
      if (track.Name.toLowerCase() === title.toLowerCase()) {
        foundSong = track;
      }
    });
  });
